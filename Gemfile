...
gem "acts-as-taggable-on", "~> 7.0", ">= 7.0.0" # A tagging plugin for Rails applications that allows for custom tagging along dynamic contexts
gem "acts_as_follower", github: "thepracticaldev/acts_as_follower", branch: "master" # Allow any model to follow any other model
gem "addressable", "~> 2.8", ">= 2.8.0" # A replacement for the URI implementation that is part of Ruby's standard library
...
gem "ancestry", "~> 3.0", ">= 3.0.7" # Ancestry allows the records of a ActiveRecord model to be organized in a tree structure
gem "autoprefixer-rails", "~> 9.7" # Parse CSS and add vendor prefixes to CSS rules using values from the Can I Use website
gem "aws-sdk-lambda", "~> 1.39", ">= 1.39.0" # Official AWS Ruby gem for AWS Lambda
...
gem "devise", "~> 4.7", ">= 4.7.1" # Flexible authentication solution for Rails
gem "dogstatsd-ruby", "~> 4.8" # A client for DogStatsD, an extension of the StatsD metric server for Datadog
gem "doorkeeper", "~> 5.3", ">= 5.3.2" # Oauth 2 provider
gem "dry-struct", "~> 1.2" # Typed structs and value objects
gem "elasticsearch", "~> 7.6" # Powers DEVs core search functionality
gem "email_validator", "~> 2.0", ">= 2.0.1" # Email validator for Rails and ActiveModel
...
gem "field_test", "~> 0.4", ">= 0.4.0" # A/B testing
gem "figaro", "~> 1.1" # Simple, Heroku-friendly Rails app configuration using ENV and a single YAML file
gem "fog-aws", "~> 3.6", ">= 3.6.3" # 'fog' gem to support Amazon Web Services
...
gem "inline_svg", "~> 1.7", ">= 1.7.2" # Embed SVG documents in your Rails views and style them with CSS
gem "jbuilder", "~> 2.10", ">= 2.10.0" # Create JSON structures via a Builder-style DSL
...
gem "nokogiri", "~> 1.13", ">= 1.13.9" # HTML, XML, SAX, and Reader parser
gem "octokit", "~> 4.18", ">= 4.18.0" # Simple wrapper for the GitHub API
...
gem "puma", "~> 4.3", ">= 4.3.12" # Puma is a simple, fast, threaded, and highly concurrent HTTP 1.1 server
gem "pundit", "~> 2.1", ">= 2.1.0" # Object oriented authorization for Rails applications
gem "pusher", "~> 1.3" # Ruby library for Pusher Channels HTTP API
gem "pusher-push-notifications", "~> 1.1", ">= 1.1.0" # Pusher Push Notifications Ruby server SDK
...
gem "sidekiq-unique-jobs", "~> 7.0.13" # Ensures that Sidekiq jobs are unique when enqueued
...
gem "sprockets", "~> 4.2", ">= 4.2.0" # Sprockets is a Rack-based asset packaging system
gem "staccato", "~> 0.5" # Ruby Google Analytics Measurement
gem "storext", "~> 3.3", ">= 3.3.0" # Add type-casting and other features on top of ActiveRecord::Store.store_accessor
gem "stripe", "~> 5.21" # Ruby library for the Stripe API
gem "strong_migrations", "~> 0.6", ">= 0.6.4" # Catch unsafe migrations
gem "timber", "~> 3.0" # Great Ruby logging made easy
gem "timber-rails", "~> 1.0", ">= 1.0.1" #  Timber integration for Rails
gem "twilio-ruby", "~> 5.34", ">= 5.34.0" # The official library for communicating with the Twilio REST API
gem "twitter", "~> 7.0", ">= 7.0.0" # A Ruby interface to the Twitter API
gem "uglifier", "~> 4.2" # Uglifier minifies JavaScript files
gem "ulid", "~> 1.2" # Universally Unique Lexicographically Sortable Identifier implementation for Ruby
gem "validate_url", "~> 1.0", ">= 1.0.8" # Library for validating urls in Rails
gem "webpacker", "~> 5.1.1" # Use webpack to manage app-like JavaScript modules in Rails

group :development do
  gem "better_errors", "~> 2.8", ">= 2.8.0" # Provides a better error page for Rails and other Rack apps
  ...
  gem "derailed_benchmarks", "~> 1.7", ">= 1.7.0", require: false # A series of things you can use to benchmark a Rails or Ruby app
  gem "erb_lint", "~> 0.0.32", require: false # ERB Linter tool
  gem "fix-db-schema-conflicts", "~> 3.0", ">= 3.0.3" # Ensures consistent output of db/schema.rb despite local differences in the database
...
group :development, :test do
  ...
  gem "bullet", "~> 6.1", ">= 6.1.0" # help to kill N+1 queries and unused eager loading
  gem "capybara", "~> 3.32", ">= 3.32.1" # Capybara is an integration testing tool for rack based web applications
  ...
  gem "rspec-rails", "~> 4.0", ">= 4.0.0" # rspec-rails is a testing framework for Rails 3+
  gem "rubocop", "~> 0.82", ">= 0.82.0", require: false # Automatic Ruby code style checking tool
  gem "rubocop-performance", "~> 1.5", ">= 1.5.2", require: false # A collection of RuboCop cops to check for performance optimizations in Ruby code
  gem "rubocop-rails", "~> 2.5", ">= 2.5.2", require: false # Automatic Rails code style checking tool
  gem "rubocop-rspec", "~> 1.38", ">= 1.38.1", require: false # Code style checking for RSpec files
...
group :test do
  ...
  gem "factory_bot_rails", "~> 5.2", ">= 5.2.0" # factory_bot is a fixtures replacement with a straightforward definition syntax, support for multiple build strategies
  gem "launchy", "~> 2.5", ">= 2.5.0" # Launchy is helper class for launching cross-platform applications in a fire and forget manner.
  ...
  gem "webdrivers", "~> 4.3", ">= 4.3.0" # Run Selenium tests more easily with install and updates for all supported webdrivers
  gem "webmock", "~> 3.8", ">= 3.8.3" # WebMock allows stubbing HTTP requests and setting expectations on HTTP requests
  gem "zonebie", "~> 0.6.1" # Runs your tests in a random timezone
end

group :doc do
  gem "sdoc", "~> 1.1", ">= 1.1.0" # rdoc generator html with javascript search index
end
